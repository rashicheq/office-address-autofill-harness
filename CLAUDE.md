...
> **Office name/area independence + search-bar polish (2026-08):** two corrections on top of the search-sheet flow above, both from Rashi: (1) Office name is Career Info context and must never drift to whatever office/area was last picked in the sheet — a bug where `handleSheetResolve` overwrote it on every resolve is fixed, and the confirm screen now shows **two independent rows**, "Office name" and "Area" (the latter only when a locality/sparse match was selected, i.e. `sparseNotice`), each editable with its own re-search button (`Change location` / `Search area`) that reopens the sheet seeded with *that row's own value* — re-searching one never touches the other. (2) The sheet's search input is now a pill with a search icon next to a **"📍 Search area" CTA that sits beside the bar itself**, not buried in a paragraph below, shown whenever the current query returns zero suggestions ("office not on Google Maps under this name") — clicking it clears the query and refocuses the input so pivoting to an area search reads as a continuation of the same search, not a dead end. `OfficeSearchSheet`'s `officeName` prop is renamed `initialQuery` to reflect that it now seeds from either field; `onResolve(result, meta)` gained a second `meta` argument (`{label, kind}`) so the caller can tell an area pick apart from an office pick without inspecting the result shape.
