...
**FR-2.3** Vibe→destination mapping needs a data source. Flagging honestly: this is either (a) manually curated by ops for a launch set of ~100–150 destinations, or (b) LLM-inferred from travel content/reviews at query time. (a) is accurate but doesn't scale past launch; (b) scales but needs a confidence/verification layer or it will confidently recommend a "chill beach" destination that's actually a container port. **Recommend (a) for V1 launch markets, explicitly flagged as a scale ceiling (OQ-2).**
...
**FR-3.2** Personal holiday-calendar import: user uploads their org's yearly leave/holiday calendar (PDF). System extracts **dates and holiday/leave-type labels only** — see OQ-4 on why we don't retain or need anything else in that document (colleague names, approval chains, etc. that a company calendar PDF often contains). Output: a ranked list of "best windows" cross-referencing personal leave availability + destination timing signal + price signal.
...
**Phase 2:** International legal/documentation module (§5) + multi-destination itineraries (FR-1.2) + group/multi-traveler support (§4.4, §4.6 shared tracker).
**Phase 3:** Deal optimizer + holiday-calendar PDF import (§4.3) — sequenced last because it has the highest data-dependency risk (OQ-3) and is not on the product's differentiated critical path (tracker is).
...
- **OQ-1 — Booking depth.** Aggregator/deep-link only (this PRD's assumption) vs. actual transactional booking-of-record. Changes licensing, liability, and payments scope entirely. Needs an explicit call before Phase 2.
- **OQ-2 — Vibe-tag data source at scale.** Curated (accurate, doesn't scale) vs. LLM-inferred (scales, needs a verification layer). Proposing curated for launch markets; revisit once destination count exceeds what ops curation can sustain.
- **OQ-3 — Live fare data feasibility/cost.** Is real fare-level pricing in budget, or does the deal optimizer ship on historical/seasonal indices only? This changes what "best time to go" can honestly claim.
- **OQ-4 — Holiday-calendar PDF retention policy.** Proposing extract-and-discard (dates/labels only, don't retain the source PDF or any other content in it). Needs privacy/legal sign-off, not just a product default.
- **OQ-5 — Confirmation-email import for the tracker.** High-value (removes manual re-entry) but requires email/inbox access scope — a real trust ask of users. Flag as a distinct feature decision, not a given.
- **OQ-6 — Real-time collaborative editing for group trips.** Scoped out of V1 (§4.6) in favor of async shared tracker. Revisit if P3 feedback says async isn't enough.
- **OQ-7 — Legal/visa content liability model.** Who signs off that this module ships with a "as of" disclaimer instead of a guarantee? This is a legal/compliance decision, not a product one — flagging so it doesn't get decided by default via silence.
- **OQ-8 — Monetization model.** Affiliate/referral on booking deep-links vs. subscription vs. freemium tracker. Not resolved here because it depends on OQ-1; noting the dependency so it isn't tackled independently and then contradicted later.
